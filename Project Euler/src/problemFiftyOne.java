...
public class problemFiftyOne {
	...
	public static void main(String[] args) {
		...
		for (int u = n; u <= 9; u++) {
			Array.set(factors, placeholder[1], (double) u);
			for (int x = n; x <= 9; x++) {
				Array.set(factors, placeholder[2], (double) x);
				for (int y = 1; y <= 1; y++) {
					Array.set(factors, placeholder[3], (double) y);
					 for (int i = 1; i <= 2; i++) {
						Array.set(factors, placeholder[4], (double) i);
						 for (int j = n; j <= 9; j++) {
							Array.set(factors, placeholder[3], (double) j);
							 for (int k = n; k <= 9; k++) {
								...
								int primeCounter = 0;
								 for (int l = n; l <= 9; l++) {
									Array.set(factors, placeholder[0], (double) l);
									//Array.set(factors, placeholder[0], (double) l);
									int factor = 0;
